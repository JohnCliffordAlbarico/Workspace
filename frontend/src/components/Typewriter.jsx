...
const Typewriter = memo(({ className = '' }) => {
  ...
  return (
    <p className={`text-sm italic text-center min-h-[40px] flex items-center justify-center ${className}`} style={{ color: '#d4a574', fontFamily: "'Cinzel', serif" }}>
      <span>"{displayText}</span>
      <span
        className="inline-block w-[2px] h-[14px] ml-[2px] align-middle animate-blink"
        style={{ background: '#d4a574', opacity: isTyping ? 1 : 0 }}
        aria-hidden="true"
      />
      <span>"</span>
    </p>
